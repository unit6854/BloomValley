...
type GalleryProps = {
  onAddToCart?: (name: string) => void; // keep this optional and compatible with your parent
};
...
function Gallery({ onAddToCart }: GalleryProps) {
  ...
  function readCart(): string[] {
    ...
    try {
      ...
      // If bad JSON ever gets in there, reset to empty for safety
      sessionStorage.removeItem(STORAGE_KEY);
      return [];
    }
  }

  function writeCart(items: string[]) {
    if (!safeIsBrowser()) return;
  ...
  // Optional: if you expect cart to change from other tabs, sync on visibility
  useEffect(() => {
  ...
  function handleAddToCart(name: string) {
    addToCart(name);
    onAddToCart?.(name); // keep your existing parent-state integration
    ...
    <div style={{ backgroundColor: '#ffffff', minHeight: '100vh' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '64px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '48px' }}>
          ...
          <button
            ...
            style={{ padding: '8px 24px', border: '2px solid #79512E', backgroundColor: '#E0D2BD', color: '#79512E', fontFamily: 'Arial', fontWeight: 'bold', cursor: 'pointer' }}
            aria-label={`View Cart (${cartItems.length})`}
          >
              ...
              <tr key={plant.id} style={{ backgroundColor: '#ffffff' }}>
                ...
                <td style={{ border: '2px solid #79512E', padding: '16px' }}>
                  <button
                    ...
                    style={{ padding: '8px 16px', backgroundColor: '#E0D2BD', color: '#79512E', fontFamily: 'Arial', fontWeight: 'bold', border: '2px solid #79512E', cursor: 'pointer' }}
                  >
                    Add to Cart
                  </button>
                </td>
      ...
      {showCart && (
        <div
          ...
          style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        >
          <div style={{ backgroundColor: '#ffffff', border: '2px solid #79512E', padding: '32px', width: '500px', maxWidth: '90%', position: 'relative' }}>
            <button onClick={() => setShowCart(false)} aria-label="Close cart" style={{ position: 'absolute', top: '12px', right: '16px', background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#00231C' }}>
              X
            </button>
            <h2 id="cart-title" style={{ fontFamily: 'Arial', fontWeight: 'bold', fontSize: '25px', color: '#790D1D', marginBottom: '24px' }}>Your Cart</h2>
            {cartItems.length === 0 ? (
              <p style={{ fontFamily: 'Calibri', fontSize: '16px', color: '#00231C' }}>Your cart is empty.</p>
            ) : (
              <ul style={{ marginBottom: '24px', paddingLeft: '20px' }}>
                {cartItems.map((item, index) => (
                  <li key={`${item}-${index}`} style={{ fontFamily: 'Calibri', fontSize: '16px', color: '#00231C', marginBottom: '8px' }}>
                    {item}
                  </li>
                ))}
              </ul>
